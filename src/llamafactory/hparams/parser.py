...
def get_train_args(args: Optional[Dict[str, Any]] = None, yaml_path=None) -> _TRAIN_CLS:
    # model_args, data_args, training_args, finetuning_args, generating_args = _parse_train_args(args)

    if yaml_path:
        parser = HfArgumentParser(_TRAIN_ARGS)
        path = os.path.abspath(yaml_path)
        model_args, data_args, training_args, finetuning_args, generating_args = parser.parse_yaml_file(path)
    else:
        model_args, data_args, training_args, finetuning_args, generating_args = _parse_train_args(args)
