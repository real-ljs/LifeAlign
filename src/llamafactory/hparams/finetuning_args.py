...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    use_l2p: bool = field(
        default=False,
        metadata={"help": "Whether to use learning to prompt."}
    )
    loss_func: str = field(
        default= "DPO",
        metadata={"help":"Loss func."}
    )
    gamma_base: float = field(
        default=2.0,
        metadata={"help": "The base focusing parameter `gamma` for the SC-FPO loss. Controls the baseline suppression of easy samples."}
    )
    # lambda_reg: float = field(
    #     default=0.5,
    #     metadata={"help": "The coefficient for the Reg Loss."}
    # )
    denoising_threshold: float = field(
        default=0.9,
        metadata={"help": "The energy preservation threshold for SVD denoising of the raw update vector. (e.g., 0.99 means keeping components that capture 99% of the energy). Set to 1.0 to disable denoising."}
    )
    projection_gamma: float = field(
        default=0.1,
        metadata={"help": "The memory plasticity factor (damping factor) for the forgetting subspace projection. 0 means full projection (maximum stability), while values closer to 1.0 allow more interference."}
    )
    CL_method: str = field(
        default="vanilla",
        metadata={"help": "The continual learning method to use. Options: vanilla, MTL, EWC, etc."}
    )
    use_replay: bool = field(
        default=False,
        metadata={"help": "Whether to use experience replay."}
    )
    ewc_lambda: float = field(
        default=0.1,
        metadata={"help": "The lambda parameter for EWC."}
    )
    replay_buffer_size: int = field(
        default=3000,
        metadata={"help": "The size of the experience replay buffer."}
    )
