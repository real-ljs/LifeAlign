...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    ...
    def calculate_metrics(self, filepath):
        ...
        def cal_metrics(preds, labels):
            ...
            preds = [pred.split("\n")[0] for pred in preds]
            # print(preds)
            tp = sum([1 for pred, label in zip(preds, labels) if pred == label == "correct"])
            fp = sum([1 for pred, label in zip(preds, labels) if pred == "correct" and label == "incorrect"])
            fn = sum([1 for pred, label in zip(preds, labels) if pred == "incorrect" and label == "correct"])
            tn = sum([1 for pred, label in zip(preds, labels) if pred == label == "incorrect"])
            # print (tp, fp, fn, tn, len(labels))
            print(f"TP: {tp}, FP: {fp}, FN: {fn}, TN: {tn}, Total: {len(labels)}")
            precision = tp / (tp + fp) if (tp + fp) > 0 else 0
            recall = tp / (tp + fn) if (tp + fn) > 0 else 0
            f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0
            accuracy = (tp + tn) / len(labels) if len(labels) > 0 else 0
            accuracy_for_pos = tp / (tp + fn) if (tp + fn) > 0 else 0
            accuracy_for_neg = tn / (tn + fp) if (tn + fp) > 0 else 0
            return {
                "precision": round(precision, 4),
                "recall": round(recall, 4),
                "f1": round(f1, 4),
                "accuracy": round(accuracy, 4),
                "accuracy_for_pos": round(accuracy_for_pos, 4),
                "accuracy_for_neg": round(accuracy_for_neg, 4),
            }
        preds, labels = [], []
    ...
    def save_predictions(
        ...
        with open(output_prediction_file, "w", encoding="utf-8") as writer:
            ...
            writer.write("\n".join(res))
        # metrics=self.calculate_metrics(output_prediction_file)
        # metric_path = os.path.join(self.args.output_dir, "metrics.jsonl")
        # self.save_data(metrics, metric_path)
