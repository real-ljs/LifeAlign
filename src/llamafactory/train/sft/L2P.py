...
class L2P:
    ...
    def __init__(self,
        ...
        prompt_weights_on_device = prompt_weights.to(device=self.device, dtype=self.model_dtype)
    ...
    def load_prompt_pool(self, path: str):
        ...
        with torch.no_grad():
            prompt_pool_data = state_dict["prompt_pool"].to(dtype=self.model_dtype)
            self.prompt_pool.copy_(prompt_pool_data)
    ...
    def _prepare_inputs(self, input_ids: torch.LongTensor, attention_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any]]:
        ...
        # Ensure input_embeds has the same dtype as model
        if input_embeds.dtype != self.model_dtype:
            input_embeds = input_embeds.to(dtype=self.model_dtype)
        ...
        # Ensure prompt_pool is in the correct dtype
        prompt_pool = self.prompt_pool.to(dtype=self.model_dtype)
        
        prompt_keys = torch.mean(prompt_pool, dim=1)
        ...
        if prompt_keys_norm.dtype != query_embeds_norm.dtype:
            query_embeds_norm = query_embeds_norm.to(dtype=prompt_keys_norm.dtype)

        similarity = torch.matmul(query_embeds_norm, prompt_keys_norm.t())
        
        _, top_k_indices = torch.topk(similarity, k=self.top_k, dim=1)

        batch_size = input_embeds.shape[0]
        selected_prompts = prompt_pool[top_k_indices]
        selected_prompts = selected_prompts.view(batch_size, self.top_k * self.prompt_length, -1)
        
        # Ensure selected_prompts has the same dtype as input_embeds
        if selected_prompts.dtype != input_embeds.dtype:
            selected_prompts = selected_prompts.to(dtype=input_embeds.dtype)
