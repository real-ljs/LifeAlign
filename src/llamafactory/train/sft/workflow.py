...
from .CLtrainer import CLTrainer
from .L2P import L2P
from .custom_data_collactor import DataCollatorForSeq2Seq
...
def run_sft(
    ...
    dataset_module = get_dataset(
        template, model_args, data_args, training_args, stage="sft", **tokenizer_module, merge=False
    )
    ...
    l2p_manager = None
    if finetuning_args.use_l2p:
        # Determine the base model for L2P
        if hasattr(model, "base_model"):
            base_model = model.base_model
        else:
            base_model = model
        
        l2p_manager = L2P(model=base_model)
    ...
    if training_args.do_predict:
        if finetuning_args.use_l2p:
            trainer.continual_eval(task_checkpoint_dir=model_args.adapter_name_or_path)
        else:
            trainer.continual_eval()